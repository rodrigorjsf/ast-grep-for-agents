...
#         TO_STATE_DIR    harness-agnostic state-dir umbrella for the PROJECT scope only
#                         (default: .claude). The Cursor host sets it to .cursor; an
#                         explicit PROJECT_CONFIG still wins. The GLOBAL scope is HOME-rooted
#                         and is deliberately NOT state-dir'd (the global default never moves).
...
#                         (default: ${TO_STATE_DIR}/tool-optimizer.local.json)
...
# GLOBAL scope is HOME-rooted and deliberately NOT state-dir'd (don't surprise the global scope).
GLOBAL_CONFIG="${GLOBAL_CONFIG:-${HOME}/.claude/tool-optimizer/config.json}"
PROJECT_CONFIG="${PROJECT_CONFIG:-${STATE_DIR}/tool-optimizer.local.json}"
